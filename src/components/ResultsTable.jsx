      ...
      <table className="data-table">
        <thead>
          <tr>
            ...
            <th>Website / Lead Info</th>
            <th style={{ width: '130px' }}>Category</th>
            <th style={{ textAlign: 'center', width: '130px' }}>Speed Scores</th>
            ...
            if (!item.success) {
              return (
                <tr key={item.id} style={{ background: 'var(--status-critical-bg)' }}>
                  ...
                  <td colSpan={6} style={{ color: 'var(--status-critical)', fontSize: '0.8rem' }}>
            ...
            return (
              <React.Fragment key={item.id}>
                <tr className={mScore < 50 ? 'row-critical' : ''}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                        ...
                        {orig.email && (
                          <span className="badge badge-cyan" style={{ fontSize: '0.7rem' }}>
                            <Mail size={10} style={{ marginRight: '3px' }} /> {orig.email}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }} title="Google PSI Mobile Performance">
                        <div className={`score-badge ${mScoreClass}`} style={{ width: '38px', height: '38px', fontSize: '0.95rem' }}>
                          {mScore}
                        </div>
                        <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '2px' }}>
                      ...
                      {dScore !== null && (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }} title="Google PSI Desktop Performance">
                          <div className={`score-badge ${dScoreClass}`} style={{ width: '38px', height: '38px', fontSize: '0.95rem' }}>
                            {dScore}
                          </div>
                          <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '2px' }}>
                  ...
                  {/* Core Web Vitals */}
                  <td>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(65px, 1fr))', gap: '0.35rem 0.65rem' }}>
                      <div className="cwv-chip">
                        <span className="cwv-label">LCP (Mobile)</span>
                        <span className={`cwv-val ${item.metrics?.lcp?.status}`}>
                          {item.metrics?.lcp?.display || 'N/A'}
                        </span>
                      </div>
                      <div className="cwv-chip">
                        <span className="cwv-label">TBT (Block)</span>
                        <span className={`cwv-val ${item.metrics?.tbt?.status}`}>
                          {item.metrics?.tbt?.display || 'N/A'}
                        </span>
                      </div>
                      <div className="cwv-chip">
                        <span className="cwv-label">FCP (Paint)</span>
                        <span className={`cwv-val ${item.metrics?.fcp?.status}`}>
                          {item.metrics?.fcp?.display || 'N/A'}
                        </span>
                      </div>
                      <div className="cwv-chip">
                        <span className="cwv-label">CLS (Shift)</span>
                  ...
                  <td>
                    <div style={{ maxWidth: '240px' }}>
                      <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.825rem', lineHeight: '1.3' }}>
                        {item.topBottleneck}
                      </div>
                    </div>
                  </td>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    {mScore < 50 ? (
                      <span className="priority-pill hot">
                        <Flame size={12} fill="var(--status-critical)" />
                  ...
                  {/* Action Buttons: Pitch, Copy, Delete */}
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.35rem' }}>
                      ...
                      <button
                        ...
                        onClick={() => onOpenPitchDrawer(item)}
                      >
                        <Mail size={12} />
                        <span>Pitch</span>
                      </button>
                      ...
                      <button
                        type="button"
                        className="btn btn-secondary"
                        style={{ fontSize: '0.75rem', padding: '0.35rem 0.45rem' }}
                      ...
                      <button
                        type="button"
                        className="btn btn-secondary"
                        style={{ fontSize: '0.75rem', padding: '0.35rem 0.45rem', color: 'var(--status-critical)' }}
                        onClick={() => onDeleteSingle(item.id)}
                ...
                {isExpanded && (
                  <tr>
                    <td colSpan={11} style={{ background: 'var(--bg-primary)', padding: '1.25rem 1.5rem 1.5rem 2.5rem', borderBottom: '1px solid var(--border-subtle)' }}>
